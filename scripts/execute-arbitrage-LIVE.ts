// scripts/execute-arbitrage-LIVE.ts
// 
// ==========================================================
// ---!!! RUNNING IN LIVE PROFIT-SEEKING MODE !!!---
// ==========================================================
//
// Purpose: Monitor for and execute PROFITABLE trades
// - Submits via Flashbots sendBundle for inclusion
// - Only submits if simulated profit is > MIN_PROFIT_USDC
// - Expects to PAY GAS from searcher wallet on success
...
// STEP 9: Looking for trades with simulated PROFIT
// Set this to a value high enough to cover gas and leave profit
const MIN_PROFIT_USDC = ethers.utils.parseUnits('2.00', 6); // Look for at least +$2.00 profit
...
async function main() {
  ...
  console.log('!!! WARNING: RUNNING IN LIVE PROFIT-SEEKING MODE !!!');
  console.log(`Will submit trades with simulated profit > +${ethers.utils.formatUnits(MIN_PROFIT_USDC, 6)} USDC`);
  console.log('Ensure contract has some USDC funded (~10 USDC) as a failsafe.');
  ...
  console.log(`Bot started in LIVE MODE. Deployer: ${deployerWallet.address}`);
  ...
  while (true) {
    try {
      ...
      try {
        ...
        // --- PROFIT CHECK (LIVE) ---
        if (profitLoss.gt(MIN_PROFIT_USDC)) {
          console.log(
            `   * RICH Path found! Sim P/L: +${ethers.utils.formatUnits(
              profitLoss,
              6
            )} USDC (Profit > ${ethers.utils.formatUnits(MIN_PROFIT_USDC, 6)})`
          );
        ...
        } else if (profitLoss.gt(0)) {
          console.log(
            `   -> Rich path profit too small: +${ethers.utils.formatUnits(
              profitLoss,
              6
            )} USDC (skipping)`
          );
          ...
          console.log(
            `   -> Rich path shows loss: ${ethers.utils.formatUnits(
              profitLoss,
              6
            )} USDC`
          );
      ...
      if (!txFound) {
        try {
          ...
          if (profitLoss.gt(MIN_PROFIT_USDC)) {
            console.log(
              `   * CHEAP Path found! Sim P/L: +${ethers.utils.formatUnits(
                profitLoss,
                6
              )} USDC (Profit > ${ethers.utils.formatUnits(MIN_PROFIT_USDC, 6)})`
            );
            ...
            console.log(
              `   -> Cheap path profit too small: +${ethers.utils.formatUnits(
                profitLoss,
                6
              )} USDC (skipping)`
            );
             ...
             console.log(
              `   -> Cheap path shows loss: ${ethers.utils.formatUnits(
                profitLoss,
                6
              )} USDC`
            );
      ...
      if (txFound && populatedTx) {
        console.log(
          `   -> PROFITABLE trade found! Expected P/L: +${ethers.utils.formatUnits(
            simulatedProfitLoss,
            6
          )} USDC`
        );
        ...
        // Use a reasonable priority fee for a profitable (MEV) bundle
        const priorityFee = ethers.utils.parseUnits('1.0', 'gwei');
        populatedTx.maxFeePerGas = block.baseFeePerGas!.add(ethers.utils.parseUnits('5', 'gwei'));
        ...
        // Case 2: Simulation reverted (this is BAD!)
        if (isSimulationRevert(simulation)) {
          ...
          console.error('   -> This is a potential bug if it happens often.');
          continue;
        }
        ...
        try {
          ...
          if (bundleResolution === FlashbotsBundleResolution.BundleIncluded) {
            ...
            if (receipt && receipt.status === 1) {
              console.log('\n' + '='.repeat(60));
              console.log('   -> ✅✅✅ PROFIT! Transaction was included and succeeded on-chain!');
              ...
              console.log('='.repeat(60));
              // --- NO process.exit(0) --- Bot continues to run
            } else {
            ...
            console.log('   -> ⚠️  Bundle was not included in target block. (Competitors were faster or more profitable)');
